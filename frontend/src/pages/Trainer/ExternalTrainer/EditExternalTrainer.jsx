...
import { FiArrowLeft, FiUser, FiMail, FiPhone, FiCalendar, FiBriefcase, FiLock } from 'react-icons/fi';
import { GiSkills } from 'react-icons/gi';

const EditExternalTrainer = () => {
  ...
  const fetchTrainerData = async () => {
    try {
      ...
      const skillIds = trainer.skills ? trainer.skills.map(s => s.skillId) : [];
      
      // Format dates for input fields
      const formattedData = {
        employeeName: trainer.employeeName || '',
        employeeEmail: trainer.employeeEmail || '',
        employeePhone: trainer.employeePhone || '',
        companyName: trainer.companyName || '',
        accessStartDate: trainer.accessStartDate ? trainer.accessStartDate.split('T')[0] : '',
        accessEndDate: trainer.accessEndDate ? trainer.accessEndDate.split('T')[0] : '',
      ...
      toast.error(error.response?.data?.message || 'Failed to load trainer data');
  ...
  const validateForm = () => {
    ...
    // Validate password if provided (optional but should be strong if changed)
  ...
  const hasChanges = () => {
    ...
    // Compare all fields except password (password is always considered changed if not empty)
    const hasPasswordChange = formData.password && formData.password.trim() !== '';
    const hasOtherChanges = JSON.stringify({
      ...
      skills: formData.skills.sort()
    }) !== JSON.stringify({
      employeeName: originalData.employeeName,
      employeeEmail: originalData.employeeEmail,
      employeePhone: originalData.employeePhone,
      companyName: originalData.companyName,
      accessStartDate: originalData.accessStartDate,
      accessEndDate: originalData.accessEndDate,
      skills: originalData.skills.sort()
    });
    
    return hasPasswordChange || hasOtherChanges;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.message || 'Failed to update external trainer';
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="mb-6">
          ...
          <h1 className="text-3xl font-bold text-gray-800">Edit External Trainer</h1>
          <p className="text-gray-600 mt-1">Update trainer details and access period</p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4">
              <h3 className="text-sm font-semibold text-red-900 mb-3 flex items-center gap-2">
                <FiLock /> Change Password (Optional)
              </h3>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
              ...
              <p className="mt-2 text-xs text-red-700">
                ⚠️ Only enter a new password if you want to change it. Must be at least 8 characters with uppercase, lowercase, and number.
              </p>
            </div>
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              ...
              <p className="mt-2 text-xs text-blue-700">
                The trainer can login only during this period. Extending the end date will give continued access.
              </p>
            </div>

            {/* Warning if dates changed */}
            {originalData && (
              (formData.accessStartDate !== originalData.accessStartDate || 
               formData.accessEndDate !== originalData.accessEndDate) && (
              <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                <h3 className="text-sm font-semibold text-yellow-900 mb-2">⚠️ Access Period Changed</h3>
                <p className="text-xs text-yellow-800">
                  You are modifying the access period. The trainer's login access will be updated accordingly.
                </p>
              </div>
            ))}
...
export default EditExternalTrainer;
