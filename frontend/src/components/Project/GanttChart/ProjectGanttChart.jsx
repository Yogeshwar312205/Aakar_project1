...
const ProjectGanttChart = () => {
  ...
  const tasks = useMemo(() => {
    ...
    // Recursive function to build substage tasks with proper parent hierarchy
    const buildRecursiveSubstages = (parentId, parentTaskId, stageId) => {
      if (!substages) return []
      const children = substages.filter(
        (ss) =>
          ...
          !ss.historyOf &&
          (parentId
            ? ss.parentSubstageId === parentId
            : !ss.parentSubstageId)
      )

      let result = []
      for (const substage of children) {
        const ssTask = {
          id: `substage-${substage.substageId}`,
          start: parseDate(substage.startDate),
          end: parseDate(substage.endDate),
          name: substage.stageName || substage.substageName,
        ...
        const childTasks = buildRecursiveSubstages(
          substage.substageId,
          ssTask.id,
          stageId
        )
        result = result.concat(childTasks)
      }
      return result
    }
    ...
    const stageTasks = activeStages?.map((stage) => {
      ...
      // Build substage tasks recursively (top-level substages have no parentSubstageId)
      const substageTasks = buildRecursiveSubstages(
        null,
        stageTask.id,
        stage.stageId
      )
