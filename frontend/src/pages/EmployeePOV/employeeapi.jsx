...
export const fetchEmployeeTrainings = async (employeeId) => {
  try {
    const response = await axios.get(`http://${IP}:3000/employee/${employeeId}`);
...
export const fetchEmployeeSkillMatrix = async (employeeId) =>{
  try {
    const response = await axios.get(`http://${IP}:3000/employeeSkill/${employeeId}`);
    return response;
  } catch (error){
...
export const fetchAttendanceForSession = async (employeeId, sessionId) => {
  try {
    const response = await axios.get(
      `http://${IP}:3000/api/attendance?employeeId=${employeeId}&sessionId=${sessionId}`
    );
